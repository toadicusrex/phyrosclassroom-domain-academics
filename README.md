# PhyrosClassroom Academics Domain Service

`phyrosclassroom-domain-academics` is the first downstream domain-service repository for PhyrosClassroom.
...
- `PhyrosClassroom.Academics.Host.CommandApi` exposes command-side endpoints
- `PhyrosClassroom.Academics.Host.QueryApi` exposes query-side endpoints
- `PhyrosClassroom.Academics.Composition` is the composition root
- local persistence uses file-backed infrastructure for early development only

## Local Development Notes

- command-side routes are under `/command/academics`
- query-side routes are under `/query/academics`
